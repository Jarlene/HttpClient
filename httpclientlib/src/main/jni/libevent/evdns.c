...
struct server_reply_item {
	...
	u16 rrclass : 16; /* The RR class (usually CLASS_INET) */
	u32 ttl; /* The RR TTL */
...
#define TEST_NAME \
	...
	for (i = 0; i < answers; ++i) {
		...
		if (type == TYPE_A && rrclass == CLASS_INET) {
		...
		} else if (type == TYPE_PTR && rrclass == CLASS_INET) {
			...
			reply.have_answer = 1;
			break;
		} else if (type == TYPE_AAAA && rrclass == CLASS_INET) {
			int addrcount, addrtocopy;
...
request_parse(u8 *packet, int length, struct evdns_server_port *port, struct sockaddr *addr, socklen_t addrlen)
{
	...
	server_req = (struct server_request*)malloc(sizeof(struct server_request));
	...
	server_req->base.questions = (struct evdns_server_question **)malloc(sizeof(struct evdns_server_question *) * questions);
	...
	for (i = 0; i < questions; ++i) {
		...
		q = (struct evdns_server_question*)malloc(sizeof(struct evdns_server_question) + namelen);
		if (!q)
			goto err;
		q->type = type;
		q->dns_question_class = rrclass;
...
evdns_request_data_build(const char *const name, const int name_len,
    const u16 trans_id, const u16 type, const u16 rrclass,
...
evdns_add_server_port(int socket, int is_tcp, evdns_request_callback_fn_type cb, void *user_data)
{
	...
	if (!(port = (struct evdns_server_port*)malloc(sizeof(struct evdns_server_port))))
		return NULL;
...
/* exported function */
int
evdns_server_request_add_reply(struct evdns_server_request *_req, int section, const char *name, int type, int rrclass, int ttl, int datalen, int is_name, const char *data)
{
	...
	item = (struct server_reply_item*)malloc(sizeof(struct server_reply_item));
	if (!item)
		return -1;
	item->next = NULL;
...
overflow:
	...
	if (!(req->response = (char*)malloc(req->response_len))) {
